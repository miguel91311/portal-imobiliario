...
const VALID_CITIES = ['luanda', 'benguela', 'lubango'];
...
export async function generateMetadata({ params }: { params: { city: string; typology: string } }): Promise<Metadata> {
  ...
  return {
    title: `${typology} em ${cityFormatted} · Portal Premium Angola`,
    description: `Encontre ${typology} para comprar ou arrendar em ${cityFormatted}, Angola. Propriedades premium com simulação IPU/Sisa.`,
  };
}
...
export default function SEOCityTypologyPage({ params }: { params: { city: string; typology: string } }) {
  ...
  return <MarketExplorarPage market="AO" />;
}
